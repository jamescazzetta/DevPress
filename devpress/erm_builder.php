...
function set_table($args){
	...
	if ($hyr != $args['hyrarchical']){
		if ($args['hyrarchical']) {
			...
			$sql = "ALTER TABLE $tablename ADD lineage VARCHAR(250) Default '0'";
			mysql_query($sql);
			
			$sql = "ALTER TABLE $tablename ADD deep INT Default '0'";
			mysql_query($sql);
			...
			$sql = "ALTER TABLE $tablename DROP COLUMN lineage";
			mysql_query($sql);
			
			$sql = "ALTER TABLE $tablename DROP COLUMN deep";
			mysql_query($sql);
