...
function set_table($args){
	...
	$create_parent_id = ($args['hyrarchical'] ? 'parent_id INT Default \'0\',' : '');
	$create_child_ids = ($args['hyrarchical'] ? 'children_ids INT,' : '');
...
function add_table_data($args){
	...
	//DEFAULTS
	if (! array_key_exists('name'			, $args)) { return = 'No Table name given'; exit;}
	if (! array_key_exists('hyrarchical'	, $args)) { $args['hyrarchical'] = FALSE;}
	if (! array_key_exists('table_data'		, $args)) { $args['table_data'] = array();}		
		...
		switch ($args['relation']) {
			...
			case 'm2m':
				...
				if (!$result){ 
					...
					mysql_query($sql);
					error_log( "<p class='log log_" . (mysql_affected_rows() != -1 ? 'success' : 'failed') . "'><date>[" . date('d-m-Y  G:i:s') . "]</date> Table <strong>$tables[0]2$tables[1]</strong> created.</p>", 3, "infos.log");
				}
			break;
		}
}
...
get_tweentable_name($table1, $table2, $prifixified = TRUE){
	switch ($prifixified) {
		case FALSE:
